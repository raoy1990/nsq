...
func TestPausing(t *testing.T) {
	...
	msg = NewMessage(topic.GenerateID(), 0, []byte("test body2"))
	topic.PutMessage(msg)
	...
	msg = NewMessage(topic.GenerateID(), 0, []byte("test body3"))
	topic.PutMessage(msg)
...
func TestOutputBuffering(t *testing.T) {
	...
	topic := nsqd.GetTopic(topicName)
	msg := NewMessage(topic.GenerateID(), 0, make([]byte, outputBufferSize-1024))
	topic.PutMessage(msg)
...
func TestSnappy(t *testing.T) {
	...
	topic := nsqd.GetTopic(topicName)
	msg := NewMessage(topic.GenerateID(), 0, msgBody)
	topic.PutMessage(msg)
...
func TestClientMsgTimeout(t *testing.T) {
	...
	topic.PutMessage(NewMessage(topic.GenerateID(), 0, make([]byte, 100)))
...
func benchmarkProtocolV2Sub(b *testing.B, size int) {
	...
	topic := nsqd.GetTopic(topicName)
	for i := 0; i < b.N; i++ {
		msg := NewMessage(topic.GenerateID(), 0, msg)
		topic.PutMessage(msg)
	}
	topic.GetChannel("ch")
	b.SetBytes(int64(len(msg)))
